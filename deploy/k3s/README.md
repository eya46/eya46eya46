...
模板默认使用 80。主机 Nginx 占用 80/443 时，在服务器副本中改为 4322：
...
Traefik Service 应只暴露 `4322/TCP`。用防火墙禁止公网直接访问 4322。
...
哪吒 WebSocket 不由 k3s 管理；Nginx/CDN 应单独处理 `/api/v1/ws/server`，不要将它转发到 4322。

```sh
sudo nginx -t
sudo systemctl reload nginx
curl -i https://www.eya46.com/healthz
curl -I https://www.eya46.com/
```
